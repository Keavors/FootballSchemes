...
(async () => {
  ...
  const N = e.board().frames.length;
  const at = k => k + '/' + N;
  t.ok('показ открыт на первом шаге', step() === at(1), step());
  t.ok('вопроса пока нет', !card());
  e.click(fwd());
  await e.wait(40);
  t.ok('перед шагом появился вопрос', !!card() && /Куда открывается ЛЗ/.test(e.text(card())));
  t.ok('шаг при этом не перелистнулся', step() === at(1), step());
  ...
  await e.wait(60);
  t.ok('после ответа шаг перелистнулся', !card() && step() === at(2), step());
  e.click(fwd());
  await e.wait(60);
  t.ok('дальше листается как обычно', step() === at(3), step());
  ...
  await e.press('Показ');
  await e.wait(1500);
  t.ok('автопоказ остановился на вопросе', !!card() && step() === at(1), step() + (card() ? ', вопрос показан' : ', вопроса нет'));
  await e.press('Показать ответ', card());
  await e.press('Дальше', card());
  await e.wait(120);
  t.ok('после ответа показ продолжился', !card() && step() !== at(1), step());
  await e.press('Закрыть', e.$('.ed-preview .te-top'));
  await e.wait(20);
