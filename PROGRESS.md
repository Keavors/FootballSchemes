...
Тесты: `npm test` (jsdom, 918 проверок).
...
Все этапы плана (0–7) сделаны и закрыты тестами: `npm test` — 918 проверок, 0 падений.

## Пример «Маятник» переписан с нуля (2026-09-13)

- Оборона средним блоком 4-2-1 и «принцип маятника»: крайний со стороны мяча выходит, трое страхуют лесенкой.
- Соперник на каждой схеме стоит по фазе игры: в атаке — на нашей половине, при розыгрыше — у своих ворот.
- Требования по позициям — слайд с ролями; две паузы-вопроса; 17 слайдов, 11 схем.
- Логику закрепляет `tests/sample.test.js`.
